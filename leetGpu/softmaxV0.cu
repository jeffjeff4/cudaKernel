...
__global__ void max_kernel(const float *x, int rows, int cols, int coarse_factor, float *max_values) {
    ...
    for (int i=0; i<coarse_factor; ++i) {
        auto col = blockIdx.x * (blockDim.x * coarse_factor) + tid + i * blockDim.x;
        if (col < cols) {
...
__global__ void sum_kernel(const float *x, float* y, int rows, int cols, int coarse_factor, const float *max_values, float *sum_values) {
    extern __shared__ float smem[];
    auto tid = threadIdx.x;
    auto row = blockIdx.y;
...
extern "C" void solve(const float* input, float* output, int N) {
    ...
    auto rows = 1;
    auto cols = N;
    auto coarse_factor = 32;
    dim3 threads(256);
    dim3 blocks(cdiv(cols, threads.x * coarse_factor), rows);
    ...
    dim3 threads0(1);
    set_max_and_sum_kernel<<<cdiv(rows, threads0.x), threads>>>(max_values, sum_values, rows);

    max_kernel<<<blocks, threads, smem_size>>>(input, rows, cols, coarse_factor, max_values);
    sum_kernel<<<blocks, threads, smem_size>>>(input, output, rows, cols, coarse_factor, max_values, sum_values);
