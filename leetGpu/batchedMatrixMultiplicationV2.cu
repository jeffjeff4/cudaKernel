...
// --- Include the provided kernel code here or ensure it's in the same file ---
// (Paste the code you provided above here)

// CPU Matrix Multiplication for Verification
void cpu_sgemm(const float* A, const float* B, float* C, int BATCH, int M, int N, int K) {
...
int main() {
    // 1. Setup Parameters
    // Note: The specialized kernel is hardcoded for 8192x6144x4096.
    // For general testing, we'll use a smaller size that meets alignment requirements.
    ...
    std::cout << "Testing SGEMM Kernel" << std::endl;
    std::cout << "Dimensions: M=" << M << ", N=" << N << ", K=" << K << ", Batch=" << BATCH << std::endl;

    size_t size_A = BATCH * M * K * sizeof(float);
    size_t size_B = BATCH * K * N * sizeof(float);
    size_t size_C = BATCH * M * N * sizeof(float);

    // 2. Host Allocation and Initialization
    std::vector<float> h_A(BATCH * M * K);
    std::vector<float> h_B(BATCH * K * N);
    std::vector<float> h_C_gpu(BATCH * M * N);
    std::vector<float> h_C_cpu(BATCH * M * N);
    ...
    double tflops = (ops * 1e-12) / (avg_ms * 1e-3);

    // 6. Verification
    /*
    std::cout << "Verifying against CPU..." << std::endl;
    CUDA_CHECK(cudaMemcpy(h_C_gpu.data(), d_C, size_C, cudaMemcpyDeviceToHost));
    
    // We only verify a small subset or a smaller matrix to save time
    cpu_sgemm(h_A.data(), h_B.data(), h_C_cpu.data(), BATCH, M, N, K);

    float max_error = 0.0f;
    for (int i = 0; i < h_C_gpu.size(); ++i) {
        max_error = std::fmax(max_error, std::fabs(h_C_gpu[i] - h_C_cpu[i]));
    }
